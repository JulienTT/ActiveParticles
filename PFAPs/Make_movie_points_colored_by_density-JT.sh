...
if [[ $# -ne 5 ]] ; then
    echo 'usage: file Lx Ly size time_increment'
    exit 0
fi
...
# NF<2 {if(iread==1) {i+=1;iread=0;file=sprintf("_data%0'"$pad"'d",i)}}
...
awk 'BEGIN{iread=0;i=0;t=0;t_increment='"$dt"';eps=0.000001;file=sprintf("_data%0'"$pad"'d",i)}
NF<2  {if(iread==1) {i+=1;iread=0;t+=t_increment;file=sprintf("_data%0'"$pad"'d",i)}}
NF>2 {if($1>t-eps) {iread=1;print $1,$2,$3,$4,$5,$6 >> file}}
' $file


ratio=$(echo $Lx $Ly | awk '{print $2/$1}')
for i in _data*;
do
    ...
    pl "$i" us 3:4:(size):(\$6) w circles lc palette
    set output
EOF
done

ffmpeg -y -r 10 -framerate 10 -i _data%0"$pad"d.png -b:a 16M -vcodec libx264 "$file"-gnuplot.mp4 
