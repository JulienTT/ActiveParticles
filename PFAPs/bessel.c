/*
  ...
  This algorithm contains the Bessel functions needed to simulate the
  chemotactic problem in 2D.
  
*/

//Returns the modified Bessel function I 1 ( x ) for any real x .
inline float bessi1(float x){
  float ax,ans;
  double y;
  if ((ax=fabs(x)) < 3.75) {
    y=x/3.75;
...
// Returns the modified Bessel function K 1 ( x ) for positive real x .
inline float bessk1(float x){
